...
SELECT distinct bb.name, a.company, b.num
 FROM route a JOIN route b ON (a.company = b.company AND a.num = b.num)
              JOIN stops aa ON (a.stop = aa.id)
              JOIN stops bb ON (b.stop = bb.id)
...
FROM
(SELECT bb.name, a.num, a.company
 FROM route a JOIN route b ON (a.company = b.company AND a.num = b.num)
              JOIN stops aa ON (a.stop = aa.id)
              JOIN stops bb ON (b.stop = bb.id)
WHERE aa.name = 'Craiglockhart' AND bb.name != 'Lochend') as first
      JOIN
(SELECT cc.name, c.num, c.company
...
WHERE cc.name != 'Craiglockhart' AND dd.name = 'Lochend') as second
 ON (first.name = second.name)
ORDER BY first.num, first.company, first.name, second.num, second.company
